...
export default function AuthGate({ onSignInWithGoogle, loading }) {
  ...
  const handleSignIn = async () => {
    setSigningIn(true);
    try {
      // Full-page redirect flow. The page navigates away, so we won't
      // unmount this component here — the next page load handles the rest.
      ...
      <main className="flex-1 flex items-center justify-center px-6 sm:px-8">
        <div className="w-full max-w-sm">
          <div className="auth-card flex flex-col items-center text-center">
            ...
            {inIframe ? (
              // v0 preview / any embedded context: Google OAuth can't run
              // inside an iframe, so send the user to a full-page tab.
              <>
                <button
                  type="button"
                  onClick={openInNewTab}
                  className="auth-google-btn mt-10 w-full h-11 rounded-full border border-border bg-card hover:bg-accent text-sm flex items-center justify-center gap-2 transition-colors cursor-pointer"
                >
                  <span>Open app in a new tab to sign in</span>
                  <svg
                    width="14"
                    height="14"
                    viewBox="0 0 24 24"
                ...
                <p className="mt-6 text-xs leading-relaxed text-muted-foreground text-balance">
                  Google blocks sign-in inside embedded previews.
                  Once signed in there, this preview will pick up your session.
                </p>
              </>
            ) : (
              // Normal flow: dev server or deployed app
              <>
                <button
                  type="button"
