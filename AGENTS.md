...
- Pure helpers (slicing, prompt/schema, merge/dedupe, id-resolve, row-build) live in `scripts/lib/review-extract.mjs` (unit-tested); the model call is isolated in `scripts/lib/review-providers.mjs` with an injectable `fetch`. Default model `gemini-2.5-pro`, swappable via the `REVIEW_MODEL` env / repo variable. **Pro needs Google AI billing** (its free tier 429s), so the practical free default is `gemini-2.5-flash`. Flash runs with **thinking disabled** and output capped (`MAX_OUTPUT_TOKENS`) so one full-doc call stays under undici's ~300s socket timeout (a longer call surfaces as `fetch failed`); the slice bisect absorbs any truncation. Network-level fetch failures are retried, and a global min-interval throttle (`MIN_CALL_INTERVAL_MS`) keeps calls under Flash's free-tier **20 requests/minute** limit no matter how far the bisect fans out.
