...
**Course names:** runtime catalog hydration removes trailing Roman numerals from course names (for example, `Elementary Chinese II` displays as `Elementary Chinese`). Course IDs/codes remain unchanged and distinguish sequence levels; source catalog records retain their official names.
...
- **Readability and overflow:** Header layouts switch at `lg` and desktop controls wrap when space is tight. Anchor the plan switcher to its left edge; keep menus within the viewport. Course names wrap, metadata uses full muted contrast, course titles and catalog add actions are native buttons, and scrollable lists retain visible thin scrollbars. Modal headers/filters do not shrink; custom forms scroll within the modal. Catalog filter selects use consistent 40px heights, 12px text/chevron insets, and a centered custom chevron while retaining native select behavior. Mobile filters use a full-width department row plus two columns and 16px inputs to avoid focus zoom.
...
- Incomplete requirement rows start a destination-selection mode on the existing semester grid; its instruction bar sits above `.planner-board-scroll` so it stays visible without overlapping sticky year headings, with a separate requirement label and Cancel action; choosing a semester opens the course picker filtered to matching courses. This shortcut view shows a separate requirement summary, the destination semester, and “Change semester” (preserves the requirement); hide the redundant catalog/custom tabs and general filters while retaining search.
...
- **StudyAwayPicker:** single-column modal (readiness banner, semester rows, native site selects, collapsed policy/advising notes); on mobile it behaves as a bottom sheet with sticky actions. Sophomore Spring is an optional study-away term and does not satisfy the required semester; at least one selected term must be from Junior Fall through Senior Fall. New York and Abu Dhabi remain unavailable in Sophomore Spring. Preserve the status-first flow: pick semesters → resolve pending sites → review warnings. The CS/DS advisory (max 3 major courses per study-away semester, advising notes) triggers when *either* major is `cs` or `data-science`, and per-major `studyAwayNotes` from both majors are shown.
- Study-away readiness is shown above the semester rows; rows label whether they are optional or fulfill the requirement, inactive rows hide site controls, and Done is disabled only while a selected semester is missing its site. Requirement warnings persist after closing because selections save immediately.
